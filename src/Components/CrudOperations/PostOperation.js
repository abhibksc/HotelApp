...
export const userRegister = async (payload) => {
  ...
  if (mail !== "" && pass !== "") {
    ...
    if (data.error) {
      ...
      alert("Succesfully Ragistered")
      
  
      localStorage.setItem("token", JSON.stringify(DataStore));
...
export const CreatePostOperation = async (payload) => {
  const updatePost_Id = async ({
    ...
    let response = await fetch(
      `${DbUrl}${userId}/createpost/${PostId.name}.json`,
      {
        method: "PUT",
        body: JSON.stringify({
  ...
  const { userName, Profileimage, userId, HandlePostedImages, Caption } =
    payload;
  console.log(HandlePostedImages);

  const CaptionPost = async ({
    Imgtoken,
    imgName,
    userName,
    Caption,
  ...
  if (HandlePostedImages) {
    try {
      ...
      const PostId = await CaptionPost({
        Imgtoken: response.data.downloadTokens,
        imgName: response.config.data.name,
        userName,
        Caption,
        userId,
      });
...
export const PostComment = async ({
  ...
  const updateCommentId = async ({
    ...
    const data = await fetch(
      `${DbUrl}${post_userId}/createpost/${post_postId}/Comment/${Comment_ID}.json`,
  ...
  const data = await fetch(
    `${DbUrl}${post_userId}/createpost/${post_postId}/Comment.json`,
...
export const PostReply = async ({
  ...
  const updateReplyId = async ({
    ...
    const data = await fetch(
      `${DbUrl}${post_userId}/createpost/${post_postId}/Comment/${Comment_ID}/Replies/${Reply_ID}.json`,
  ...
  const data = await fetch(
    `${DbUrl}${post_userId}/createpost/${post_postId}/Comment/${Comment_ID}/Replies.json`,
  ...
  const data_ = await updateReplyId({
    Reply,
    Replier_Profile_Pic,
    Replier_UserName,
    Replier_UserId,
